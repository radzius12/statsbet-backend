// StatsBet Backend with Email Verification
...
db.serialize(() => {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS users (
    ...
    initial_amount REAL DEFAULT 200,
  ...
  db.run(`ALTER TABLE bets ADD COLUMN profile_id TEXT DEFAULT 'default'`, (err) => {
    if (err && !err.message.includes('duplicate column name')) {
      console.error('Migration error:', err);
    } else {
      console.log('✅ Profile support added to database');
    }
  });
}); // ← TEN NAWIAS MUSI BYĆ!
...
app.post('/api/bets', authenticateToken, (req, res) => {
  const { date, betType, betCategory, odds, stake, potentialWin, result, profileId } = req.body;
  ...
  db.run(
    `INSERT INTO bets (user_id, date, bet_type, bet_category, odds, stake, potential_win, result, profile_id) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [req.user.userId, date, betType, betCategory, odds, stake, potentialWin, result || null, profileId || 'default'],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
...
app.put('/api/bets/:id', authenticateToken, (req, res) => {
  ...
  db.run(
    `UPDATE bets SET date = ?, bet_type = ?, bet_category = ?, odds = ?, 
     stake = ?, potential_win = ?, result = ?, profile_id = ?
     WHERE id = ? AND user_id = ?`,
    [date, betType, betCategory, odds, stake, potentialWin, result, profileId || 'default', betId, req.user.userId],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
  ...
  db.all(query, params,
    (err, bets) => {
      ...
      db.get('SELECT initial_amount FROM users WHERE id = ?', [req.user.userId], (err, user) => {
        ...
        const initialAmount = user ? user.initial_amount : 200;
